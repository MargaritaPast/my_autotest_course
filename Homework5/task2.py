...
def repeats(our_str):
    dict = {}
    new_str = ''
    for i in enumerate(our_str):  # Разбиваю строку на пары индекс-значение
        if i[1] in dict:  # Проверяю, есть ли ключ в словаре
            dict[i[1]] += 1
        else:
            dict[i[1]] = 1  # Преобразование в словарь, изменение местами индекса и значения
        new_str += str(i[1]) + '_' + str(dict[i[1]])  # Преобразование в строку и форматирование
