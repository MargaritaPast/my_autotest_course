...
def everything_for_your_cat(cats_data):
    ...
    human = ''
    cat = ''
    for i in enumerate(cats_data):
        human = str(i[1][2] + ' ' + i[1][3])  # Канкатинация Имя + Фамилия
        cat = str(i[1][0] + ', ' + str(i[1][1]))  # Канкатинация Кличка котика + возраст
        if human in dict:  # Проверяю, есть ли ключ в словаре
            dict[human] = dict[human] + '; ' + cat  # Добавляю текущего котика
        else:
            dict[human] = cat  # Иначе создаю ключ с котиком
    our_str = ''
    for j in dict.items():
        our_str += j[0] + ': ' + j[1] + '\n'  # Собираю отформатированную строку вывода
    return our_str
