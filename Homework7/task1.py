...
class Segment:
    def __init__(self, point1, point2):  #Класс Segment
        self.x1 = point1[0]
        self.y1 = point1[1]
        self.x2 = point2[0]
        self.y2 = point2[1]
   ...
   #@classmethod
    def length(self):
        length = ((self.x1 - self.x2) ** 2 + (self.y1 - self.y2) ** 2) ** 0.5  #Формула длины отрезка
        return round(length, 2)

    def x_axis_intersection(self):
        if self.x1 >= 0 and self.x2 <= 0 or self.x2 >= 0 and self.x1 <= 0:  #Условие пересечения оси x
            return True
        else:
            return False

    def y_axis_intersection(self):
        if self.y1 >= 0 and self.y2 <= 0 or self.y2 >= 0 and self.y1 <= 0:  #Условие пересечения оси y
            return True
        else:
            return False
