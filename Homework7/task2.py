...
class PersonInfo:
    def __init__(self, full_name: str, age: int, *subdivision: str):  #Класс PersonInfo
        self.full_name = full_name
        self.age = age
        self.subdivision = subdivision

    def short_name(self):
        return self.full_name.split(' ')[1] + ' ' + self.full_name.split(' ')[0][0] + '.'  #Преобразование "Имя Фамилия" к "Фамилия И."

    def path_deps(self):
        return ' --> '.join(self.subdivision)  #Путь "Головное подразделение --> ... --> Конечное подразделение"


    def new_salary(self):
        text = ''.join(self.subdivision)
        count = sorted([(text.count(letter), letter) for letter in set(text)], reverse = True)
        salary = 1337 * self.age * (count[0][0] + count[1][0] + count[2][0])  #Формула зарплаты
        return salary
