...
class PublicTransport:
    ...
    def __init__(self, brand: str, engine_power: int, year: int, color: str, max_speed: int):  #Класс PublicTransport
        self.brand = brand
    ...
    @property
    def info(self):  #Свойство выводит на печать информацию о: марке, цвете, годе выпуска, мощности двигателя
        print(f'Brand: {self.brand} Color: {self.color} Year: {self.year} Power: {self._engine_power}')
        return (f'Brand: {self.brand} Color: {self.color} Year: {self.year} Power: {self._engine_power}')


class Bus(PublicTransport):

    def __init__(self, brand, engine_power, year, color, max_speed, passengers, park, fare):  #Класс Bus, унаследованный от PublicTransport
    ...
    @property
    def park(self):  #Свойство возвращает значение - парк приписки автобуса
        return self.park

    @park.setter
    def park(self, number):  #Проверяет, что номер парка лежит в диапазоне
        assert 1000 <= number <= 9999
        self.__park = number

class Tram(PublicTransport):  #Класс Tram, унаследованный от PublicTransport
    ...
    @property
    def how_long(self):  #Свойство вычисляет время прохождения маршрута по формуле max_speed/(4*path)
